...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    // Auth (uses your existing Supabase session bearer)
    let userId: string | null = null;
    try { userId = requireUser(req).userId; } catch { return bad(res, 401, 'Missing or invalid bearer token'); }

    // Body
    const body = typeof req.body === 'string' ? JSON.parse(req.body) : (req.body || {});
    let { text, sourceLang = 'auto', targetLang, model = 'madlad-400' }:
        { text?: string; sourceLang?: string; targetLang?: string; model?: string } = body;
    ...
    // Accept BOTH env name pairs (yours + the original)
    const RUNPOD_TRANSLATE_URL =
        process.env.RUNPOD_TRANSLATE_URL || process.env.MADLAD_RUNPOD_URL;
    const RUNPOD_TRANSLATE_TOKEN =
        process.env.RUNPOD_TRANSLATE_TOKEN || process.env.MADLAD_API_KEY;
    const RUNPOD_TRANSLATE_TIMEOUT = Number(process.env.RUNPOD_TRANSLATE_TIMEOUT || 90000);

    // If not configured, return a stub (your file already did this)
    if (!RUNPOD_TRANSLATE_URL || !RUNPOD_TRANSLATE_TOKEN) {
        ...
        const { error } = await supabaseAdmin.from('translations').insert({
            user_id: userId, source_lang: normSource, target_lang: normTarget,
            input_text: text, output_text: output, model,
        });
        if (error) return bad(res, 500, error.message);
        return res.status(200).json({ output, raw: { stub: true } });
    }

    // Forward to your Runpod FastAPI
    const input = {
        task: 'translate',
        model,
        text,
        source_lang: normSource,   // 'auto' is allowed
    ...
    try {
        ...
        // Normalize output field names from the pod
        const output =
